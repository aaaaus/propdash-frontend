//LeaseInfo is a child of UnitDetail that renders information about any past, current and future leases on the unit.
...
const LeaseInfo = props => {
  ...
  const selectOptions = props.allResidents.map(resident => ({
    label: `${resident.first_name} ${resident.last_name}`,
    value: resident.id
  }));

  const pastLeases = props.leases.filter(
    lease => lease.unit_id === props.unit.id && lease.status === "past"
  );
  const currentLease = props.leases.filter(
    lease => lease.unit_id === props.unit.id && lease.status === "current"
  )[0];
  const futureLease = props.leases.filter(
    lease => lease.unit_id === props.unit.id && lease.status === "future"
  )[0];
  ...
  switch (props.type) {
    case "past":
      if (pastLeases.length > 0) {
        ...
        const startDate = new Date(
          pastLeases[0].start_date * 1000
        ).toLocaleDateString();
        const endDate = new Date(
          pastLeases[0].end_date * 1000
        ).toLocaleDateString();
    ...
    case "current":
      if (currentLease) {
        ...
        const startDate = new Date(
          currentLease.start_date * 1000
        ).toLocaleDateString();
        const endDate = new Date(
          currentLease.end_date * 1000
        ).toLocaleDateString();
        ...
        return (
          <div id="current" className="lease-info">
            ...
            <button
              className="button-toggle-notice"
              onClick={props.handleToggleNotice}
            >
              Toggle Notice
            </button>
            <br />
            <button className="button-move-out" onClick={props.handleMoveOut}>
              Move Out
            </button>
            <br />
          ...
          <div className="lease-info">
            ...
            */}
            <form onSubmit={props.handleCreateNewLease}>
              ...
              />
              <br />
              <button className="button-create-lease" type="submit">
                Create New Future Lease
              </button>
            </form>
          </div>
        );
    ...
    case "future":
      if (futureLease) {
        ...
        const startDate = new Date(
          futureLease.start_date * 1000
        ).toLocaleDateString();
        const endDate = new Date(
          futureLease.end_date * 1000
        ).toLocaleDateString();
        ...
        return (
          <div id="future" className="lease-info">
            ...
            <button className="button-move-in" onClick={props.handleMoveIn}>
              Move In
            </button>
            <br />
