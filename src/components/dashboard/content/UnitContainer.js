...
//This component contains the sort buttons and the grid of units itself, along with all sorting and filtering logic
...
class UnitContainer extends React.Component {
  ...
  componentDidMount() {
    this.props.fetchUnits(); //fetches ALL units in db
    this.props.fetchLeases(); //fetches ALL leases in db
    this.props.fetchResidents(); //fetches ALL residents in db
  }

  //helper function for following function (sortUnitsBy)
  dateHelper = unit => {
    const datesArray = unit.leases.map(lease => {
      if (lease.status !== "future") {
        //do not include future leases
  ...
  //first sort: will sort by unit number or lease end date
  sortUnitsBy = array => {
    if (this.state.sortBy === "number") {
      //sort by unit number
      const num = array[0].number; //test first element to see what type of array this is
      if (isNaN(parseFloat(num.charAt(num.length - 1)))) {
        //for apartments with "10A" format
        const sortedArray = array.sort(function(unit1, unit2) {
          //sort by leading number
          if (
            parseInt(unit1.number.slice(0, -1)) >
            parseInt(unit2.number.slice(0, -1))
          )
            return 1;
          if (
            parseInt(unit1.number.slice(0, -1)) <
        ...
        //for apartments with "901" format
        return array.sort(
          (unit1, unit2) => parseFloat(unit1.number) - parseFloat(unit2.number)
        );
      }
    } else {
      //sort by end date
  ...
  //second - if name filter is active, will filter out non-matches
  filterByName = unitArray => {
    //in comes the array of units from the filterUnits function
    /////////////
    ...
    if (this.props.filterText.length > 0) {
      ...
      return unitArray.filter(unit => {
        ...
        const leaseIds = residents.map(resident => {
          // debugger
          const lease = resident.leases.find(
  ...
  //third - returns filtered array of units based on three filters that can be turned on/off in state
  ...
  renderUnits = () => {
    if (
      ...
      const units = this.props.units.filter(
        unit => unit.property_id === property.id
      );
      const filteredUnits = this.filterUnits(
        this.filterByName(this.sortUnitsBy(units))
      );
  ...
  //used in final render function for sort buttons, changes sort state
  handleSortBy = e => {
    ...
    return (
      <div id="unit-container">
        <div id="sort-button-container">
          <span className="filter-headers">Sort by:</span>
          <br />
          <button
            id="sort-number"
        ...
        <div className="grid clearfix">{this.renderUnits()}</div>
      </div>
    );
  }
}
...
export default connect(
  mapStateToProps,
  { fetchUnits, fetchLeases, fetchResidents, selectUnit }
)(UnitContainer);
